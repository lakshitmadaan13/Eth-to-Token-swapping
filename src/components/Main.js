...
class Main extends Component {
    ...
    async loadBlockchainData() {
        ...
        if (networkId === 3) {
        const swap = new web3.eth.Contract(Swap, "0x83220562e5cf7CFf7144572698ee35f59F6DfBC1")
  ...
  //      this.state.erc20.methods.approve("0xf3D4333cae5b21ff496D08B930467A70a4e884ff", tokenBlock.tokenAmount).send({ from: this.state.account })
  //     .then(() => {
  ...
  swapToken() {
    ...
    console.log(isValid)
    if (isValid) {
      //const tokenInputAddress = tokenBlock.map((token) => token.tokenAmount)
      const tokenInputAmount = tokenBlock.map((token) => token.tokenAmount)
      let amount = tokenInputAmount[0]
      const targetTokenAddress = this.state.targetToken 
      console.log(amount)
      console.log(targetTokenAddress)
    
      this.state.swapping.methods.swap(targetTokenAddress).send({ from: this.state.account,value:amount})
        .then((res) => {
         console.log(res)
        })
        .catch((error) => {
